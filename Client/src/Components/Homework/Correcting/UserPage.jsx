...
export function UserPage({ laraPath,toolsId,attemp, course ,nextPath=""}) {
        ...
        const saveItem = () => save(url, component, method, nextPath+"/courses/"+course+"/tools/homework/"+toolsId+"/correcting"+"?"+Math.random());
        ...
        return <>
            <Box title={attempInfo?.user?.name + attempInfo?.user?.lname}>
                ...
                {Tools.getArray( attempInfo).map((answer,index)=>{
