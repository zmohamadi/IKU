...
const Message = ({message, className = "", quiz = {}}) => {
    ...
    return<>
        <Frame title = "Quiz Message">            
            <Box className="col-span-8 overflow-hidden text-lg" title={"Message"} shadow={false} >
                <div className="col-span-12" style={{overflow: "hidden1", position: "relative1", minHeight: "150px"}}>
                    ...
                    <img className="hidden sm:block absolute right-2 w-2/5 mb-1 -ml-12 " 
                        alt="" 
                        style={{top: "100px"}}
                        src={laraDomain+"/admin/Midone-v3/Icewall_v1.0.9/dist/images/phone-illustration.svg"}></img>
                </div>
            </Box>
            <Box className="col-span-4 bg-theme-17 text-white mt-5 rounded-md" title={"Quiz Info"} shadow={false} >
                <div className="col-span-12 text-md">
                    <h4><b>{quiz.title}</b></h4>
                    <div className="mt-4"><b className="mx-2">Start: </b>{quiz.start_time_date}</div>
                    <div className="mt-2"><b className="mx-2">End: </b>{quiz.end_time_date}</div>
                    {quiz.limit_time > 0? <div className="mt-2"><b className="mx-2">Limit: </b>{quiz.limit_time} minutes</div>: ""}
                </div>
            </Box>
        </Frame>
    </>
...
const ReplyItem = ({reply, component, qindex})=>{
    ...
    return <ul>
            {question?.question_type_id==2 ?<>
                    <li>
                        <Radio value="0" label={ [(parseInt(qindex)+1)+") "+ question?.title+" (score: "+question?.score+")"]} 
                    ...
                    <li>
                        <div className="font-bold font-medium" dangerouslySetInnerHTML={{ __html: qindex+1 +") "+ question?.title+" (score: "+question?.score+")" }}></div> 
                        <Textarea label="Your Answer" className="col-span-6" refItem={[component, "response_"+question?.id+"#"+question?.question_type_id]} />
                    </li>
                </>
            }
        </ul>
    ...
    return(<>
            <Frame title ="Quiz Answering" className="intro-y">
                ...
                <Box className="col-span-4 " title={quizInfo?.data?.title} shadow={false} >
                    ...
                    <h3 className="col-span-12 text-lg font-bold">Quiz overview</h3>
